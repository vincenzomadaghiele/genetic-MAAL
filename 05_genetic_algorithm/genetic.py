...
if __name__ == '__main__': 
	...
	parser.add_argument('--GENERATE_LOGS_FROM_START_CONFIG', type=bool, default=False,
						help='weight to use for weighted fitness functions')
	...
	generateLogs = args.GENERATE_LOGS_FROM_START_CONFIG # if true it runs an emulation where it finds starting cofing
	...
	sound_corpus_filepath = './05_genetic_algorithm/00_genetic_search/corpus' # dir where all the sound examples are
	corpus_output_dir_path = './05_genetic_algorithm/00_genetic_search/corpus'
	starting_config_filepath = './05_genetic_algorithm/00_genetic_search/starting_config.json'
	objective_config_filepath = './05_genetic_algorithm/00_genetic_search/objective_config.json'
	...
	if generateLogs:
		...
		for soundfile in os.listdir(sound_corpus_filepath):
			if soundfile != '.DS_Store':
				...
				command = f'python3 00_offline_MAAL/offlineMAAL.py --SOUNDFILE_FILEPATH {sound_corpus_filepath}/{soundfile}/{soundfile}.wav --CONFIG_FILEPAHT {objective_config_filepath} --OUTPUT_DIR_PATH {corpus_output_dir_path} --VERBOSE 0 --IGNORE_WARNINGS True'
	...
	# GENERAL INITS
	config_files_path = './05_genetic_algorithm/00_genetic_search/config_files'
	best_configs_path = './05_genetic_algorithm/00_genetic_search/best_configs'
	looper_outputs_path = './05_genetic_algorithm/00_genetic_search/looper_outputs'
	...
	for k in range(N_ITERATIONS):
		...
		for config_file in config_files_list:
			...
			for soundfile in os.listdir(sound_corpus_filepath):
				if soundfile != '.DS_Store':
					command = f'python3 00_offline_MAAL/offlineMAAL.py --SOUNDFILE_FILEPATH {sound_corpus_filepath}/{soundfile}/{soundfile}.wav --CONFIG_FILEPAHT {config_filepath} --OUTPUT_DIR_PATH {output_dir_path} --VERBOSE 0 --IGNORE_WARNINGS True'
					threads.append(Thread(target=call_script, args=([command])))
		[t.start() for t in threads]
		[t.join() for t in threads]
