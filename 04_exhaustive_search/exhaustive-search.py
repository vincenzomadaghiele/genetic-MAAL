...
if __name__ == '__main__': 
	...
	soundfile_filepath = './00_search/corpus/USE_CASE_1.wav'
	starting_config_filepath = './00_search/corpus/objective_config.json'
	...
	config_files_path = './00_search/config_files'
	...
	for config_file in config_files_list:
		...
		output_dir_path = f'./00_search/looper_outputs/{config_file.split(".")[0]}'
		if os.path.isdir(output_dir_path):
			shutil.rmtree(output_dir_path)
		os.mkdir(output_dir_path)

		command = f'python3 offlineMAAL.py --SOUNDFILE_FILEPATH {soundfile_filepath} --CONFIG_FILEPAHT {config_filepath} --OUTPUT_DIR_PATH {output_dir_path}'
